...
def getTotalSpend_Year(userID):
    totalSpendYear = convertSQLToDict(
        db.execute("SELECT SUM(amount) AS expenses_year FROM expenses WHERE user_id = :usersID AND YEAR(date(expensedate)) = YEAR(CURRENT_DATE)",{"usersID": userID}).fetchall()
    )
    return totalSpendYear[0]['expenses_year']

def getTotalSpend_Month(userID):
    results = db.execute(
        "SELECT SUM(amount) AS expenses_month FROM expenses WHERE user_id = :usersID AND YEAR(date(expensedate)) = YEAR(CURRENT_DATE) AND MONTH(date(expensedate)) = MONTH(CURRENT_DATE)",
...
def getTotalSpend_Week(userID):
    ...
    results = db.execute(
        "SELECT SUM(amount) AS expenses_week FROM expenses WHERE user_id = :usersID AND YEAR(date(expensedate)) = YEAR(CURRENT_DATE) AND WEEK(date(expensedate)) = WEEK(CURRENT_DATE)",
        {"usersID": userID}).fetchall()
...
def getBudgets(userID, year=None):
    ...
    if budgets_query and year in budgets_query:
        for record in budgets_query[year]:
            ...
            results = db.execute(
                "SELECT SUM(amount) AS spent FROM expenses WHERE user_id = :usersID AND YEAR(date(expensedate)) = :year AND category IN (SELECT categories.name FROM budgetcategories INNER JOIN categories on budgetcategories.category_id = categories.id WHERE budgetcategories.budgets_id = :budgetID)",
...
def getLastFourWeekNames():
    ...
    results = db.execute("SELECT DATE(DATE_SUB(CURRENT_DATE, INTERVAL WEEKDAY(CURRENT_DATE) DAY) ) startofweek, (date(DATE_SUB(CURRENT_DATE, INTERVAL WEEKDAY(CURRENT_DATE) DAY) + INTERVAL 6 DAY )) AS endofweek UNION SELECT DATE(DATE_SUB(CURRENT_DATE, INTERVAL WEEKDAY(CURRENT_DATE) DAY) - INTERVAL 1 WEEK) AS startofweek, (DATE(DATE_SUB(CURRENT_DATE, INTERVAL WEEKDAY(CURRENT_DATE) DAY) - INTERVAL 1 DAY)) AS endofweek UNION SELECT DATE(DATE_SUB(CURRENT_DATE, INTERVAL WEEKDAY(CURRENT_DATE) DAY) - INTERVAL 2 WEEK) AS startofweek, (DATE(DATE_SUB(CURRENT_DATE, INTERVAL WEEKDAY(CURRENT_DATE) DAY) - INTERVAL 8 DAY)) AS endofweek UNION SELECT DATE(DATE_SUB(CURRENT_DATE, INTERVAL WEEKDAY(CURRENT_DATE) DAY) - INTERVAL 3 WEEK) AS startofweek, (DATE(DATE_SUB(CURRENT_DATE, INTERVAL WEEKDAY(CURRENT_DATE) DAY) - INTERVAL 15 DAY)) AS endofweek ORDER BY startofweek ASC").fetchall()
...
def getWeeklySpending(weekNames, userID):
    ...
    for name in weekNames:
        ...
        results = db.execute(
            "SELECT SUM(amount) AS amount FROM expenses WHERE user_id = :usersID AND YEAR(date(expensedate)) = YEAR(date(:weekName)) AND WEEK(date(expensedate)) = WEEK(date(:weekName))",
...
def getMonthlySpending(userID, year=None):
    ...
    results = db.execute(
        "SELECT MONTH(date(expensedate)) AS month, SUM(amount) AS amount FROM expenses WHERE user_id = :usersID AND YEAR(date(expensedate)) = :year GROUP BY MONTH(date(expensedate)) ORDER BY month",
...
def getSpendingTrends(userID, year=None):
    ...
    results = db.execute("SELECT category, COUNT(category) as count, SUM(amount) as amount FROM expenses WHERE user_id = :usersID AND YEAR(date(expensedate)) = :year GROUP BY category ORDER BY COUNT(category) DESC",
                         {"usersID": userID, "year": year}).fetchall()
