...
def generateBudgetsReport(userID, year=None):
    ...
    if budgetsReport:
        for record in budgetsReport:
            ...
            results = db.execute("SELECT expenses.description, expenses.category, expenses.expenseDate, expenses.payer, expenses.amount FROM expenses WHERE user_id = :usersID AND YEAR(date(expensedate)) = :year AND category IN (SELECT categories.name FROM budgetcategories INNER JOIN categories on budgetcategories.category_id = categories.id WHERE budgetcategories.budgets_id = :budgetID)",
...
def generateMonthlyReport(userID, year=None):
    ...
    results = db.execute(
        "SELECT description, category, expensedate, amount, payer FROM expenses WHERE user_id = :usersID AND YEAR(date(expensedate)) = :year ORDER BY id ASC", {"usersID": userID, "year": year}).fetchall()
...
def generateSpendingTrendsReport(userID, year=None):
    ...
    spending_trends_chart = pet_dashboard.getSpendingTrends(userID, year)
    ...
    categories_active = pet_categories.getSpendCategories(userID)
    ...
    categories_inactive = pet_categories.getSpendCategories_Inactive(userID)
    ...
    results = db.execute(
        "SELECT MONTH(date(expensedate)) AS monthofcategoryexpense, category AS name, COUNT(category) AS count, SUM(amount) AS amount FROM expenses WHERE user_id = :usersID AND YEAR(date(expensedate)) = :year GROUP BY MONTH(date(expensedate)), category ORDER BY COUNT(category) DESC",
...
def generatePayersReport(userID, year=None):
    ...
    results_payers = db.execute(
        "SELECT payer AS name, SUM(amount) AS amount FROM expenses WHERE user_id = :usersID AND YEAR(date(expensedate)) = :year GROUP BY payer ORDER BY amount DESC", {"usersID": userID, "year": year}).fetchall()
    ...
    results_nonExpensePayers = db.execute(
        "SELECT name FROM payers WHERE user_id = :usersID AND name NOT IN (SELECT payer FROM expenses WHERE expenses.user_id = :usersID AND YEAR(date(expensedate)) = :year)", {"usersID": userID, "year": year}).fetchall()
...
def sendMail():
    msg = "Report sended to your Mail Id. Check It!"
    if sendMail(mail):
        msg = "Report sended to your Mail Id. Check It!"
    else:
