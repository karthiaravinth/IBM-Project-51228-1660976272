...
function loadMonthlySpendingChart(monthlySpending) {
    if (monthlySpending == null) {
        ...
        for (i = 0; i < monthlySpending.length; i++) {
            ...
            //console.log(monthlySpending[i].name,monthlySpending[i].amount);
        }
        // Draw the chart
    ...
    if (spendingTrends == null) {
        ...
        let spendingChart = new Chart(chartElement, {
            ...
            options: {
                ...
                title: {
                    display: true,
                    text: 'Total spent, total no of expenses, and % of overall spent per spend category'
                },
                scales: {
                    yAxes: [{
                        scaleLabel: {
                            display: true,
                            labelString: "Total no of Expenses"
                        }
                    }],
                    xAxes: [{
                        scaleLabel: {
