...
from pet_extra import apology, login_required, usd
...
load_dotenv()
db = 'mysql+pymysql://root:root@localhost:3306/ibmsample'
...
def register():
    ...
    if request.method == "POST":
        ...
        if not check_password_hash(hashedPass,con_password):
            return render_template("register.html", username1=username)  #FOR PASSWORD CHECKING CHANGE USERNAME1 VARIABLE
        now = datetime.now().strftime("%m/%d/%Y %H:%M:%S")
        db.execute("INSERT INTO users (username, hash, registerDate, lastLogin) VALUES (:username, :hashedPass, :registerDate, :lastLogin)", {"username": username, "hashedPass": hashedPass, "registerDate": now, "lastLogin": now})
        db.commit()
        newUserID = db.execute("SELECT LAST_INSERT_ID()").fetchone()[0]
        print(newUserID);
        db.commit()

        # Create default spending categories for user
        db.execute("INSERT INTO userCategories (category_id, user_id) VALUES (1, :usersID), (2, :usersID), (3, :usersID), (4, :usersID), (5, :usersID), (6, :usersID), (7, :usersID), (8, :usersID)",{"usersID": newUserID})
        db.commit()
...
def index():
    ...
    if request.method == "GET":
        ...
        categories = pet_categories.getSpendCategories(session["user_id"])

        # Get the users payers (for quick expense modal)
        payers = pet_account.getPayers(session["user_id"])

        # Get todays date (for quick expense modal)
        date = datetime.today().strftime('%Y-%m-%d')

        # Get the users income
        income = pet_account.getIncome(session["user_id"])

        # Get current years total expenses for the user
        expenses_year = pet_dashboard.getTotalSpend_Year(session["user_id"])

        # Get current months total expenses for the user
        expenses_month = pet_dashboard.getTotalSpend_Month(
            session["user_id"])

        # Get current week total expenses for the user
        expenses_week = pet_dashboard.getTotalSpend_Week(session["user_id"])

        # Get last 5 expenses for the user
        expenses_last5 = pet_dashboard.getLastFiveExpenses(
            session["user_id"])

        # Get every budgets spent/remaining for the user
        budgets = pet_dashboard.getBudgets(session["user_id"])

        # Get weekly spending for the user
        weeks = pet_dashboard.getLastFourWeekNames()
        spending_week = pet_dashboard.getWeeklySpending(
            weeks, session["user_id"])

        # Get monthly spending for the user (for the current year)
        spending_month = pet_dashboard.getMonthlySpending(
            session["user_id"])

        # Get spending trends for the user
        spending_trends = pet_dashboard.getSpendingTrends(
            session["user_id"])

        # Get payer spending for the user
        payersChart = pet_reports.generatePayersReport(session["user_id"])
    ...
    # User reached route via POST
        ...
        expenses = pet_expenses.addExpenses(formData, session["user_id"])
...
def expensehistory():
    ...
    if request.method == "GET":
        ...
        history = pet_expenses.getHistory(session["user_id"])
    ...
    # User reached route via POST
        ...
        if userHasSelected_deleteExpense == True:
            ...
            deleted = pet_expenses.deleteExpense(
            ...
            history = pet_expenses.getHistory(session["user_id"])
            categories = pet_categories.getSpendCategories(
                session["user_id"])
            payers = pet_account.getPayers(session["user_id"])
        ...
        # Update the existing expense record
            ...
            expensed = pet_expenses.updateExpense(
...
def budgets(year=None):
    ...
    if request.method == "GET":
        ...
        budgets = pet_budgets.getBudgets(session["user_id"])

        # Get the users total budgeted amount
        budgeted = pet_budgets.getTotalBudgetedByYear(
    ...
    # User reached route via POST
        ...
        # Delete the budget
        deletedBudgetName = pet_budgets.deleteBudget(
...
def createbudget():
    ...
    if request.method == "POST":
        ...
        budgetDict = pet_budgets.generateBudgetFromForm(formData)
        ...
        if "apology" in budgetDict:
            ...
            budget = pet_budgets.createBudget(
        ...
        # Get the users total budgeted amount
        budgeted = pet_budgets.getTotalBudgetedByYear(session["user_id"])
...
def updatebudget(urlvar_budgetname):
    ...
    if request.method == "POST":
        ...
        if "apology" in budgetDict:
            ...
            budget = pet_budgets.updateBudget(
    ...
    # User reached route via GET
        ...
        budgetID = pet_budgets.getBudgetID(
            urlvar_budgetname, session["user_id"])
        if budgetID is None:
            return apology("'" + urlvar_budgetname + "' budget does not exist")
        else:
            budget = pet_budgets.getBudgetByID(budgetID, session["user_id"])
        ...
        budget = pet_budgets.getUpdatableBudget(budget, session["user_id"])
...
def categories():
    ...
    if request.method == "POST":
        ...
        if userHasSelected_newCategory:
            ...
            categoryCount = len(
                pet_categories.getSpendCategories(session["user_id"]))
            if categoryCount >= 30:
            ...
            categoryID = pet_categories.getCategoryID(newCategoryName)
            ...
            if categoryID:
                ...
                existingID = pet_categories.getCategoryID(
                ...
                # Add the category to the users account
                    ...
                    pet_categories.addCategory_User(
            ...
            # Category does not exist in the DB already - create a new category and then add it to the users account
                ...
                newCategoryID = pet_categories.addCategory_DB(
                    newCategoryName)
        ...
        if userHasSelected_renameCategory:
            ...
            oldCategoryID = pet_categories.getCategoryID(oldCategoryName)
            ...
            # Check to see if the *new* name already exists in the database (None == does not exist)
            newCategoryID = pet_categories.getCategoryID(newCategoryName)
            ...
            if newCategoryID:
                ...
                newCategoryNameFromDB = pet_categories.getSpendCategoryName(
                    newCategoryID)

                # Rename the category
                pet_categories.renameCategory(
        ...
        if userHasSelected_deleteCategory:
            ...
            categoryID = pet_categories.getCategoryID(deleteName)
            ...
            # Delete the category
            pet_categories.deleteCategory(categoryID, session["user_id"])
    ...
    # User reached route via GET
        ...
        categoryBudgets = pet_categories.getBudgetsSpendCategories(
            session["user_id"])

        # Generate a single data structure for storing all categories and their associated budgets
        categoriesWithBudgets = pet_categories.generateSpendCategoriesWithBudgets(
...
def budgetsreport(year=None):
    ...
    budgets = pet_reports.generateBudgetsReport(session["user_id"], year)
...
def monthlyreport(year=None):
    ...
    monthlySpending = pet_reports.generateMonthlyReport(
        session["user_id"], year)
...
def spendingreport(year=None):
    ...
    spendingReport = pet_reports.generateSpendingTrendsReport(
        session["user_id"], year)
...
def payersreport(year=None):
    ...
    payersReport = pet_reports.generatePayersReport(
        session["user_id"], year)
...
def updateaccount():
    ...
    if request.method == "POST":
        ...
        if userHasSelected_updateIncome:
            ...
            updatedIncome = pet_account.updateIncome(
        ...
        if userHasSelected_addPayer:
            ...
            # Add the payer
            newPayer = pet_account.addPayer(newName, session["user_id"])
        ...
        if userHasSelected_renamePayer:
            ...
            # Rename the payer
            renamedPayer = pet_account.renamePayer(
        ...
        if userHasSelected_deletePayer:
            ...
            # Delete the payer
            deletedPayer = pet_account.deletePayer(name, session["user_id"])
        ...
        if userHasSelected_updatePassword:
            ...
            updatedPassword = pet_account.updatePassword(request.form.get(
        ...
        user = pet_account.getAllUserInfo(session["user_id"])
